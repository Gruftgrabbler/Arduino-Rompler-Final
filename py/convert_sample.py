...
# TODO Export the modified sample data back to a wav to check it for correctness
...
def read_sample(path: str, startswith: str, base: str):
    ...
    :param path: path to the .cpp file containing the sample data
    :param startswith: the startswith string after which the actual sample data begin's
    :param base: hex or dec
    :return: sample data as python list
    """
    isProcessing = False
    sample = []
    ...
    # return list(map(int, sample))  # Return the sample data and convert it to a list of integer
    return sample
...
class Process:
    ...
    def process(sample, input_sample_rate, output_sample_rate, input_bit_rate, output_bit_rate):
        ...
        for i in range(len(sample)):
            ...
            sample[i] = sample[i] >> shift  # Use only the highest output_bit_rate Bits of the sample
            # sample[i] = sample[i] & mask  # Use only the highest 8 Bits of the sample

        # Reduce sample rate // TODO This cuts off samples of the ratio is not a factor of the sample len
...
if __name__ == '__main__':
    ...
    # Audio Sample
    PATH = '../sample/wav2sketch/AudioSampleSample.cpp'
    STARTS_WITH = 'const unsigned int AudioSample'
    BASE = 'hex'

    # Sine Test
    # PATH = '../sample/wav2sketch/AudioSampleSineData.cpp'
    ...
    pd = pandas.DataFrame(sample_data)
    pd.to_csv('sample_data.csv')
