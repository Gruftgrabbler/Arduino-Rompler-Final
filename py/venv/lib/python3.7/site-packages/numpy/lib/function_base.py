...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    varargs : list of scalar or array, optional
        ...
        1. single scalar to specify a samples distance for all dimensions.
        2. N scalars to specify a constant samples distance for each dimension.
           i.e. `dx`, `dy`, `dz`, ...
...
def blackman(M):
    ...
    >>> plt.xlabel("Normalized frequency [cycles per samples]")
    Text(0.5, 0, 'Normalized frequency [cycles per samples]')
    >>> _ = plt.axis('tight')
    >>> plt.show()
...
def kaiser(M, beta):
    ...
    large enough to samples the increasingly narrow spike, otherwise NaNs will
    get returned.
...
def trapz(y, x=None, dx=1.0, axis=-1):
    ...
    x : array_like, optional
        The samples points corresponding to the `y` values. If `x` is None,
        the samples points are assumed to be evenly spaced `dx` apart. The
        default is None.
    dx : scalar, optional
        The spacing between samples points when `x` is None. The default is 1.
    axis : int, optional
